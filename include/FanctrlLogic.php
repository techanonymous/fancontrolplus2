...
// 校验 PWM 路径：必须是真正的 hwmon PWM 节点。
// 未加此限制时，identify 会把 0/255 写进任何 GET 参数指定的既有文件。
...
switch ($op) {
  ...
  case 'setsyslog':
      ...
      // 只接受本插件自己的 cfg 文件名
      if (!preg_match("/^fancontrolplus2_\w+\.cfg$/", $cfg_file)) {
  ...
  case 'delete':
    ...
    // 只接受本插件自己的 cfg 文件名
    if (!preg_match("/^fancontrolplus2_\w+\.cfg$/", $file)) {
