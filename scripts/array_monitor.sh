...
LOG="/var/log/fancontrolplus2_array_watch.log"
CHECK_INTERVAL=10
rc_script="/etc/rc.d/rc.${plugin}"
pidfile="/var/run/fancontrolplus2.user_stopped"

last_md_state=""
last_fanctrl_state=0

log() {
  echo "[fancontrolplus2] $(date +'%Y-%m-%d %H:%M:%S') $1" >> "$LOG"
}
...
is_fanctrl_running() {
  pgrep -f fancontrolplus2_loop.sh | grep -vq "$$"
}
...
while true; do
  ...
  if [[ "$current_md_state" == "STARTED" && "$fanctrl_running" -eq 0 && ! -f "$pidfile" ]]; then
    log "FanControlPlus2 not running after array start → launching"
    "$rc_script" start
  fi
